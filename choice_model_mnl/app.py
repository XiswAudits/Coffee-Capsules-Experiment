...
def observations():
    ...
    for h,(rc,pc) in HOUSEHOLDS.items():
        for _,r in RAW.iterrows():
            rq,pq=int(r[rc]),int(r[pc]); choice="Regular" if rq>0 else "Premium" if pq>0 else "No Purchase"
            rows.append({"Household":h,"T":int(r.T),"P_Regular":float(r.P_Regular),"P_Premium":float(r.P_Premium),"Regular":rq,"Premium":pq,"Choice":choice,"Quantity":rq+pq})
    return pd.DataFrame(rows)
OBS=observations(); PR_MEAN=OBS.P_Regular.mean(); PR_STD=OBS.P_Regular.std(ddof=0); PP_MEAN=OBS.P_Premium.mean(); PP_STD=OBS.P_Premium.std(ddof=0)
...
def fit(h):
    ...
    results=[minimize(nll,x,method="L-BFGS-B",bounds=BOUNDS) for x in starts]; res=min(results,key=lambda x:x.fun); p=res.x; probs=softmax(utility(p,s.P_Regular,s.P_Premium)); yhat=probs.argmax(1)
    at={PARAMS[i]:bool(abs(p[i]-BOUNDS[i][0])<1e-5 or abs(p[i]-BOUNDS[i][1])<1e-5) for i in range(4)}
    return {"sample":s,"params":p,"probs":probs,"predicted":yhat,"observed":y,"at_bound":at,"nll":float(res.fun)}
...
def boundaries(m):
    ar,ap,br,bp=m["params"]
    rn=PR_MEAN-ar*PR_STD/br if abs(br)>1e-12 else np.nan; pn=PP_MEAN-ap*PP_STD/bp if abs(bp)>1e-12 else np.nan
    if abs(bp)>1e-12:
        slope=br*PP_STD/(bp*PR_STD); intercept=PP_MEAN+(ar-ap)*PP_STD/bp-slope*PR_MEAN
    else: slope=intercept=np.nan
    return rn,pn,slope,intercept

def decision_map(h,pr,pp):
    m=MODELS[h]; s=m["sample"]; xs=np.linspace(30,70,180); ys=np.linspace(55,145,180); xx,yy=np.meshgrid(xs,ys)
    q=softmax(utility(m["params"],xx.ravel(),yy.ravel())); zones=q.argmax(1).reshape(xx.shape); fig=go.Figure(go.Heatmap(x=xs,y=ys,z=zones,zmin=0,zmax=2,colorscale=[[0,"rgba(24,121,78,.10)"],[.33,"rgba(24,121,78,.10)"],[.34,"rgba(217,154,25,.10)"],[.66,"rgba(217,154,25,.10)"],[.67,"rgba(102,112,133,.10)"],[1,"rgba(102,112,133,.10)" ]],showscale=False,hoverinfo="skip"))
    rn,pn,slope,intercept=boundaries(m)
    ...
    if np.isfinite(slope) and np.isfinite(intercept):
        line=slope*xs+intercept; mask=np.isfinite(line)&(line>=55)&(line<=145)
...
fig,sc,pred=decision_map(household,pr,pp); m=MODELS[household]; avg,qr,qp=qstats(m["sample"]); rn,pn,slope,intercept=boundaries(m); ar,ap,br,bp=m["params"]
...
with x: st.markdown('<div class="card"><div class="card-title">Utility</div><div class="mini">Each household has its own four fitted parameters.</div><div class="eq">V_R = ASC_R + β_R · z_R</div><div class="eq">V_P = ASC_P + β_P · z_P</div><div class="eq">V_N = 0</div></div>',unsafe_allow_html=True)
...
with z: st.markdown(f'<div class="card"><div class="card-title">Choice boundaries</div><div class="mini">Equal-utility thresholds in original price units.</div><br>Regular = Premium<br><b>P_P = {slope:.2f} × P_R + {intercept:.1f}</b><br><br>Regular = No Purchase<br><b>P_R ≈ {rn:.1f}</b><br><br>Premium = No Purchase<br><b>P_P ≈ {pn:.1f}</b></div>',unsafe_allow_html=True)
...
**Price scaling.** Prices are standardised only for numerical stability:
- Regular: `z_R = (P_R − {PR_MEAN:.2f}) / {PR_STD:.2f}`
- Premium: `z_P = (P_P − {PP_MEAN:.2f}) / {PP_STD:.2f}`

The four household-specific parameters are then estimated by **maximum likelihood**. The optimiser searches for the parameter values that assign the highest probability to the choices actually observed in the 11 weeks.

**No Purchase = 0** is a reference-alternative convention. It fixes the utility scale so the two ASCs are interpreted relative to No Purchase.

**Boundaries** come from setting two utilities equal. For example, Regular = Premium means `V_R = V_P`; solving that equation produces the straight line shown on the map.""")
with st.expander("Why MNL? And why independent households?"):
    st.markdown("""The MNL treats **Regular, Premium, and No Purchase as three competing alternatives in one Random Utility Model**, rather than fitting Buy/No-Buy and product choice as separate classifiers.

This version is intentionally **independent by household**: each household has its own utility parameters. That directly answers the question of how each household responds to the price pair.

The trade-off is sample size. Each household has only 11 observations, and Household 1/2 never observed No Purchase while Household 3 never observed Regular. The optimiser therefore uses finite parameter bounds and flags estimates that hit a bound. Those estimates are exploratory, not precise economic measurements.""")
with st.expander("How should I read the decision map?"):
    st.markdown("""- **Dots** = actual observed household-week choices.
- **Background** = the alternative with the highest estimated utility at each price combination.
- **Dashed vertical/horizontal lines** = product vs No Purchase equal-utility thresholds when they fall inside the chart.
- **Solid line** = Regular vs Premium equal-utility boundary.
- **Star** = the price scenario you are testing.

The MNL still produces probabilities, so a boundary should not be interpreted as a hard causal willingness-to-pay threshold.""")

st.subheader("Observed weeks")
d=m["sample"][["T","P_Regular","P_Premium","Choice","Quantity"]].copy(); d.columns=["Week","Regular price","Premium price","Observed choice","Quantity"]; st.dataframe(d,use_container_width=True,hide_index=True)
st.caption("Exploratory research tool · 11 observations per household · independent household MNL · not a causal elasticity or validated WTP model")
