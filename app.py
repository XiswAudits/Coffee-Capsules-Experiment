...
with st.sidebar:
    st.markdown('<div class="brand">☕ Coffee Capsule Lab</div>',unsafe_allow_html=True)
    ...
    st.divider(); household=st.selectbox("Household",list(HOUSEHOLDS)); st.markdown('<div class="navlabel">Scenario</div>',unsafe_allow_html=True); p_r=st.slider("Regular price",30,70,42); p_p=st.slider("Premium price",65,105,93); st.caption("33 household-week observations · 3 households")
...
st.markdown('<div id="model"></div>',unsafe_allow_html=True); st.write(""); x,y,z=st.columns(3)
...
The three final probabilities are:
- P(No Purchase) = 1 − P(Buy)
- P(Regular) = P(Buy) × [1 − P(Premium | Buy)]
- P(Premium) = P(Buy) × P(Premium | Buy)

For the selected scenario, the model gives **{sc["Regular"]:.1%} Regular, {sc["Premium"]:.1%} Premium, and {sc["No Purchase"]:.1%} No Purchase**.""")
with st.expander("Where do the boundary equations come from?"): st.markdown(f"""Each boundary is the 50% contour of its binary logistic model. Because the models are linear in standardised prices, setting the log-odds to zero gives a straight line in original price units.
